...
function sanitizeSettings(rawSettings = {}) {
    ...
    const defaultHalfSpacingMm = (defaults.lineSpacingMm || 0) / 2;
    const sourceLines = Array.isArray(rawSettings.lines) && rawSettings.lines.length
        ? rawSettings.lines
        : defaults.lines;

    sanitized.lines = sourceLines.map((line, index) => {
        const fallbackLine = defaults.lines[index] || { spacingTopMm: index === 0 ? 0 : defaultHalfSpacingMm, spacingBottomMm: defaultHalfSpacingMm };
...
function computeAutoBorderSize({ lines, lineSpacingMm, letterSpacingMm, paddingMm }) {
    ...
    const fallbackHalfSpacingMm = (lineSpacingMm || 0) / 2;
    ...
    if (lines.length === 0) {
        ...
        const spacingPx = mmToPx(fallbackHalfSpacingMm) * 2;
...
function addLine(initial = {}, options = {}) {
    const fallbackSpacingMm = Math.max(0, parseFloat(lineSpacingInput.value) || 0);
    const halfSpacingMm = fallbackSpacingMm / 2;
    const isFirstLine = linesContainer.children.length === 0;
    const spacingTopMm = initial.spacingTopMm != null ? initial.spacingTopMm : (isFirstLine ? 0 : halfSpacingMm);
    const spacingBottomMm = initial.spacingBottomMm != null ? initial.spacingBottomMm : halfSpacingMm;
...
function buildSvg(settings) {
    ...
    if (settings.lines.length > 0) {
        ...
        const fallbackHalfSpacingMm = (settings.lineSpacingMm || 0) / 2;
        const spacingPairsPx = settings.lines.map((line, index) => {
            const topMm = line.spacingTopMm != null ? line.spacingTopMm : (index === 0 ? 0 : fallbackHalfSpacingMm);
            const bottomMm = line.spacingBottomMm != null ? line.spacingBottomMm : fallbackHalfSpacingMm;
            return {
        ...
        const totalTextHeightPx = settings.lines.reduce((acc, _, index) => {
            return acc + spacingPairsPx[index].top + fontSizesPx[index] + spacingPairsPx[index].bottom;
        }, 0);

        let currentBaseline = originY + (heightPx - totalTextHeightPx) / 2;

        settings.lines.forEach((line, index) => {
            const fontSizePx = fontSizesPx[index];
            const spacing = spacingPairsPx[index];
            currentBaseline += spacing.top + fontSizePx;
            ...
            currentBaseline += spacing.bottom;
        });
    }
